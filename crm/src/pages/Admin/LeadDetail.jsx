...
import {
    ...
    Layers, TruckIcon, UserPlus, Users, ArrowRight
} from 'lucide-react';
import { useNavigate, useParams } from 'react-router-dom';
import { useAuth } from '../../context/AuthContext';
import {
    getLeadDetail, updateLeadStatus, updateInventory,
    finalizePrice, getUsers, assignLead, addLeadNote,
    BACKEND_URL
} from '../../utils/api';
...
const LeadDetail = () => {
    ...
    const handleStatusUpdate = async (newStatus) => {
        const isOverride = STATUS_FLOW.indexOf(newStatus) !== STATUS_FLOW.indexOf(lead.status) + 1 && newStatus !== 'CANCELLED';
        if (isOverride && !reason && currentUser.role === 'ADMIN') {
    ...
    const savePrice = async () => {
        try {
            await finalizePrice(id, tempPrice, reason || 'Manual verification');
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap' }} className="mobile-stack">
                ...
                <div style={{ display: 'flex', gap: '10px' }}>
                    {lead.status !== 'CANCELLED' && lead.status !== 'COMPLETED' && (
                        <Button variant="outline" style={{ color: '#ef4444', borderColor: '#fee2e2' }} onClick={() => handleStatusUpdate('CANCELLED')}>
                            Cancel Lead
                        </Button>
                    )}
                </div>
            ...
            <div className="grid grid-cols-3 lg:grid-cols-1" style={{ gap: '1.5rem', alignItems: 'start' }}>
                                        ...
                                        <div>
                                            ...
                                            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                                                ...
                                                <div style={{ display: 'grid', gridTemplateColumns: '120px 1fr', fontSize: '0.9rem' }}>
                                                    ...
                                                    <span style={{ fontWeight: '700' }}>AI Estimator</span>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem', position: 'sticky', top: '0px', alignSelf: 'start' }}>
                    ...
                    {currentUser.role === 'ADMIN' && (
                        <Card title="Lead Ownership" subtitle="Managing consultant for this account.">
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px', background: '#f8fafc', borderRadius: '12px', border: '1px solid #e2e8f0' }}>
                                    <div style={{ width: '40px', height: '40px', borderRadius: '50%', background: lead.assigned_to ? '#1d4ed8' : '#e2e8f0', display: 'grid', placeItems: 'center', color: lead.assigned_to ? 'white' : '#64748b', fontWeight: '800' }}>
                                        {lead.assigned_to ? lead.assigned_to.name.charAt(0) : <Users size={20} />}
                                    </div>
                                    <div style={{ flex: 1 }}>
                                        <div style={{ fontSize: '0.9rem', fontWeight: '700', color: '#1e293b' }}>{lead.assigned_to ? lead.assigned_to.name : 'Unassigned'}</div>
                                        <div style={{ fontSize: '0.75rem', color: '#64748b' }}>{lead.assigned_to ? 'Sales Consultant' : 'Available for pickup'}</div>
                                    </div>
                                </div>
                                <Button variant="outline" style={{ width: '100%', gap: '8px' }} onClick={() => setShowAssignModal(true)}>
                                    <UserPlus size={16} /> Assign Representative
                                </Button>
                            </div>
                        </Card>
                    )}
                ...
                <div style={{ position: 'fixed', inset: 0, background: 'rgba(15, 23, 42, 0.7)', backdropFilter: 'blur(4px)', display: 'grid', placeItems: 'center', zIndex: 1000, padding: '2rem' }}>
                    <Card style={{ width: '100%', maxWidth: '450px', padding: '0', overflow: 'hidden' }}>
                        <div style={{ padding: '1.5rem', borderBottom: '1px solid #e2e8f0', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                            <div>
                                <h2 style={{ fontSize: '1.25rem', fontWeight: '800', color: '#0f172a' }}>Assign Consultant</h2>
                                <p style={{ fontSize: '0.85rem', color: '#64748b' }}>Select who will manage this moving lead.</p>
                            </div>
                            <button onClick={() => setShowAssignModal(false)} style={{ background: 'none', border: 'none', color: '#64748b', cursor: 'pointer' }}><X size={24} /></button>
                        </div>
                        <div style={{ padding: '1rem', display: 'flex', flexDirection: 'column', gap: '8px', maxHeight: '400px', overflowY: 'auto' }}>
                            {[{ _id: '', name: 'Unassigned', role: 'SYSTEM' }, ...salesTeam].map((rep) => (
                                <div
                                    key={rep._id}
                                    onClick={() => {
                                        handleAssign(rep._id);
                                    ...
                                    style={{
                                        ...
                                        padding: '12px 16px',
                                        borderRadius: '12px',
                                        border: `1px solid ${(lead.assigned_to?._id || '') === rep._id ? '#1d4ed8' : '#e2e8f0'}`,
                                        background: (lead.assigned_to?._id || '') === rep._id ? '#eff6ff' : 'white',
                                        cursor: 'pointer',
                                        transition: 'all 0.2s'
                                    }}
                                >
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                                        <div style={{
                                            ...
                                            height: '36px',
                                            borderRadius: '50%',
                                            background: rep._id === '' ? '#f1f5f9' : '#1d4ed8',
                                            display: 'grid',
                                            placeItems: 'center',
                                            color: rep._id === '' ? '#64748b' : 'white',
                                            fontWeight: '800'
                                        }}>
                                            {rep.name.charAt(0)}
                                        </div>
                                        <div>
                                            <div style={{ fontSize: '0.9rem', fontWeight: '700', color: '#1e293b' }}>{rep.name}</div>
                                            <div style={{ fontSize: '0.7rem', color: '#64748b', fontWeight: '600', textTransform: 'uppercase' }}>{rep._id === '' ? 'Public Pool' : 'Sales Representative'}</div>
                                        </div>
                                    </div>
                                    {(lead.assigned_to?._id || '') === rep._id && <div style={{ color: '#1d4ed8' }}><UserCheck size={20} /></div>}
                                </div>
                            ))}
                        </div>
                        <div style={{ padding: '1.25rem', background: '#f8fafc', borderTop: '1px solid #e2e8f0', textAlign: 'center' }}>
                            <Button variant="outline" style={{ width: '100%' }} onClick={() => setShowAssignModal(false)}>Close Modal</Button>
                        </div>
                    </Card>
                </div>
            )}
                ...
                <div style={{ position: 'fixed', inset: 0, background: 'rgba(15, 23, 42, 0.7)', backdropFilter: 'blur(4px)', display: 'grid', placeItems: 'center', zIndex: 1000, padding: '2rem' }}>
                    <Card style={{ width: '100%', maxWidth: '450px', padding: '0', overflow: 'hidden' }}>
                        ...
                        <div style={{ padding: '1.5rem', display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                            ...
                            <textarea
                                placeholder="Add optional notes about this stage transition..."
                                value={reason}
